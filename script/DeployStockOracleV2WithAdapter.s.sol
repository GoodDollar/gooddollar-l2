...
/**
 * @title DeployStockOracleV2WithAdapter
 * @notice Deploys StockOracleV2 + adapter that matches PriceOracle interface.
 *         New CollateralVault deployments should use the adapter address.
 *
 * Usage (devnet):
 *   forge script script/DeployStockOracleV2WithAdapter.s.sol \
 *     --rpc-url http://localhost:8545 \
 *     --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 \
 *     --broadcast
 */
contract DeployStockOracleV2WithAdapter is Script {
    function run() external {
        ...
        // 2. Deploy the adapter (PriceOracle-compatible wrapper)
        StockOracleV2Adapter adapter = new StockOracleV2Adapter(address(oracle), deployer);
        console.log("StockOracleV2Adapter deployed at:", address(adapter));

        // 3. Register stock symbols
        ...
        for (uint256 i = 0; i < symbols.length; i++) {
            oracle.registerSymbol(symbols[i], 3600, 1000);
        }
        ...
        console.log("StockOracleV2:        ", address(oracle));
        console.log("StockOracleV2Adapter:  ", address(adapter));
        console.log("Update op-stack/addresses.json with these addresses");
    }
}
