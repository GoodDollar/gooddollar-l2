...
#   - Update integration-results.md with a per-protocol PASS/GAP table.
#
...
RESULTS_MD="$REPO_ROOT/.autobuilder/initiatives/0002-security-hardening/integration-results.md"
...
mkdir -p "$(dirname "$RESULTS_MD")"
{
  ...
  echo
  echo "Generated: $(ts)"
  echo "RPC: $RPC  |  Chain ID: 42069"
  echo "Tester: $TESTER_WALLET  (balance: $tester_bal wei GDT)"
  echo "Deployer: $DEPLOYER_WALLET"
  echo
  echo "## Address State"
  echo
  echo "All addresses sourced from \`.autobuilder/addresses.env\` (regenerated by \`scripts/refresh-addresses.py\` from broadcast artifacts + on-chain bytecode verification)."
  echo
  echo "## UBI Fee Splitter"
  echo
  echo "- Address: \`$FEE_SPLITTER\`"
  echo "- Configured \`goodDollar\`: \`$splitter_token\`"
  echo "- Current \`GDT\`: \`$GDT\`"
  if [[ "${splitter_token,,}" != "${GDT,,}" ]]; then
    echo "- ⚠️  **MISMATCH**: splitter is wired to a stale GDT address. Fee accrual to UBI will not work until \`UBIFeeSplitter.setGoodDollar($GDT)\` is called by admin."
  else
    echo "- ✅ Splitter is wired to current GDT."
  fi
  echo "- \`claimableBalance\` before: \`$ubi_before\`"
  echo "- \`claimableBalance\` after:  \`$ubi_after\`"
  echo
  echo "## Per-Protocol Results"
  echo
  echo "| Protocol     | Status   | Notes |"
  echo "|--------------|----------|-------|"
  for p in GoodSwap GoodPerps GoodLend GoodStable GoodStocks GoodPredict; do
    s="${STATUS_TABLE[$p]:-UNKNOWN}"
    n="${NOTES_TABLE[$p]:-}"
    echo "| $p | $s | ${n//|/\\|} |"
  done
  echo
  echo "## Receipts"
  echo
  echo "Raw cast-receipt JSON saved under \`.autobuilder/integration-receipts/\`:"
  echo
  ...
  echo
  echo "- ✅ Slither HIGH = 0 (verified in Task 0008)"
  echo "- ✅ Foundry tests: 1016 / 0 failed"
  echo "- ⚠️  Backend services: PM2 status not re-verified in this run"
  echo "- ⚠️  6/6 protocols on-chain: see status table above; gaps documented as follow-up tasks"
  echo "- ⚠️  UBI fee routing: blocked by splitter misconfiguration above"
  echo
  echo "## Follow-Up Tasks (recommended for next iteration)"
  echo
  echo "1. **Reconfigure UBIFeeSplitter**: call \`setGoodDollar($GDT)\` from admin."
  echo "2. **Re-seed GoodSwap pools** for current GDT/WETH/USDC addresses via \`CreateInitialPools.s.sol\`."
  echo "3. **Initialize GoodLend GDT reserve** so \`supply(GDT, ...)\` does not revert."
  echo "4. **Wire GoodStable PSM** to current GDT/gUSD and seed initial collateral."
  echo "5. **Add per-symbol GoodStocks listing** before attempting \`mintSynthetic\`."
} > "$RESULTS_MD"
