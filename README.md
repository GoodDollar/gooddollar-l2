...
> *Updated: 2026-05-16 — task 0051 (PerpEngine `_closePosition()` restructured to CEI: snapshot → effects → interaction; 40/40 GoodPerps + 1026/1026 full-suite forge tests green; `_closePosition` no longer flagged by `slither reentrancy-no-eth`; cumulative Slither MEDIUM `reentrancy-no-eth` 12→11; remaining `openPosition`/`closePosition`/`liquidate` findings tracked for the separate `funding.applyFunding`-CEI follow-up; nonReentrant guard preserved as belt-and-suspenders defense)*
