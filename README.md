...
- Stocks routes now surface an explicit wallet-connector degradation notice when `NEXT_PUBLIC_WC_PROJECT_ID` is missing/invalid, and the developer warning is deduplicated to once per browser session instead of repeating on every route import.
- Stocks ticker now canonicalizes malformed research tab query params (for example `?tab=unknown`) back to the default tab URL state, keeping shared deep-links resilient and consistent.
...
- Stocks screener state now persists in URL query params (search, filters, sort), and ticker preview links carry that state forward so browser Back from `/stocks/[ticker]` restores the exact prior screen context.
...
- Stocks markets empty-state recovery is now cause-aware: when zero results come from filters (or search + filters), the inline empty-state action clears the active constraints instead of only clearing search text.
...
- **Iter 31 — Stocks intelligence movers correctness fix.** Tightened the `Top Movers` selection logic so `Losers` shows only negative 24h movers (no positive filler rows) and `Gainers` shows only non-negative movers, with regression coverage in `frontend/src/components/stocks/__tests__/MarketIntelligencePanel.test.tsx`.
