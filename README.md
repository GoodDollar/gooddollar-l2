...
> *Updated: 2026-05-16 — task 0052 (Swap UI now honors user-configured transaction deadline: extracted `computeSwapDeadline` helper with defensive `[60s, 3600s]` clamp + sane defaults, plumbed `deadline` from `useSwapSettings` through `SwapWalletActions` → `useSwapExecute().swap()` → on-chain `swapExactTokensForTokens`, added "Auto-cancel after" row to `SwapConfirmModal` so users see the value before signing; 19 new/updated tests green; previous hardcoded 20-min override eliminated, MEV-protection slider in Swap Settings now actually controls on-chain execution window)*
