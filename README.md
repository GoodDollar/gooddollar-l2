...
> *Updated: 2026-05-16 — task 0042 (`MarketFactory.marketCreators` allowlist — unblocks autonomous GoodPredict integration test for the QA Bot's 30-min loop without rotating admin or running deployer-key workarounds; 6 new TDD tests, 51/51 predict tests green; redeployed `MarketFactory` to `0xfaA7…6CCE` and synced address across 5 config files; updated `verify-onchain-integration.sh` to grant + create idempotently; integration results now 6/6 ✅ success, 10/10 PM2 services online, UBI fee routing verified at 10.0263 wei this run; mainnet policy unchanged — empty allowlist behaves as single-admin)*
