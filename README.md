...
- Stocks onboarding and stocks portfolio disconnected flows now expose explicit fallback rails when mobile wallet QR is unavailable (`Use In-browser Wallet`, `Try Another Connector`, and `Continue in Read-only Mode`) to prevent single-path onboarding dead ends.
- Stocks portfolio disconnected state now includes pre-connect value previews (market benchmark movers + sample portfolio health/UBI cards) and clearer holdings/history tab guidance so the page remains informative before wallet connection.
- Stocks portfolio disconnected Holdings tab now prioritizes stable read-only copy over transient `Loading positions…` messaging, removing a mobile-first polish regression that looked like unresolved data fetch.
...
- Stocks detail mobile hierarchy now leads with ticker identity and price context before the trade widget, while keeping chart density trimmed and lower-priority metrics behind a `More metrics` disclosure for faster scanability.
