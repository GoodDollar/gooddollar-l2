...
> *Updated: 2026-05-16 — task 0046 (Explore — `change24h`/`volume24h`/`marketCap` now from CoinGecko via additive `Quote` type on `usePriceFeeds`; `useOnChainMarketData` consumes `quotes` instead of hardcoded zeros; G$ retains on-chain market cap derivation; 6 new tests on `usePriceFeeds`; legacy `prices` map preserved byte-for-byte for all 5 callers; 20/20 hook tests + 293 lib-suite tests green)*
