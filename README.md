...
> *Updated: 2026-05-16 — task 0039 (`CollateralVault` precision-loss sweep — replaced 18 `divide-before-multiply` patterns across 8 mint/burn/liquidate functions with OpenZeppelin `Math.mulDiv`, dropping Slither MEDIUM count 129 → 111 with no remaining findings on `CollateralVault.sol` itself; 12/12 vault tests + 46/46 stocks suite + 1020/1020 total Foundry tests still green; price/collateral/fee maths now use full-precision 512-bit intermediates)*
