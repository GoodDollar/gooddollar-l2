...
> *Updated: 2026-05-16 — task 0044 (Predict — featured market no longer duplicated below the hero card; selection lifted into pure `selectFeaturedMarket()` helper, parent dedups the grid by id with `activeMarketsWithoutFeatured`, count label adapts to "X More Active Markets" when hero is shown; 6 new unit tests cover empty/all-expired/single/highest-volume/ignore-expired/tie-stable cases; full frontend suite 430/430 green; react-doctor 96/100 on the diff)*
