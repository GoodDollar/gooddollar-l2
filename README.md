...
- Stocks onboarding primary CTA now uses high-contrast text + state styling (`text-black`, explicit hover/focus/active states) so `Connect Wallet to Trade Stocks` remains visually prominent while meeting accessibility contrast checks in browser QA.
...
- `/api/prices` now uses a keyed in-memory cache (`Map<cacheKey, {data, timestamp}>`) instead of a single global slot, eliminating cache thrashing between distinct symbol baskets (e.g. one widget asking for `ETH` and another for `USDC` no longer evict each other on every request) and emitting an `X-Cache` header (`HIT`/`MISS`/`STALE`) for observability. The keyed cache is bounded by `MAX_CACHE_ENTRIES = 16` with LRU-ish eviction via `Map` insertion order, and incoming symbol lists are now capped at `MAX_SYMBOLS_PER_REQUEST = 50` with a fast `400` rejection before any upstream fetch, closing a small DoS / cost-amplification surface. `STALE` is only ever served for the same cache key as the original `MISS`, so an upstream failure on one basket can never leak unrelated cached data from another basket. Covered by `frontend/src/__tests__/prices-route.test.ts` (7 tests: keyed HIT/MISS, per-key isolation, cross-key STALE-suppression, same-key STALE-after-TTL, oversize rejection, missing-param 400, no-mapping empty 200).
...
- **2026-05-20 — Stocks detail `next/dynamic({ ssr: false })` regression revert (lane-b task 0024).** Restored `useMounted`-style gating on `/stocks/[ticker]`: replaced the reintroduced `next/dynamic({ ssr: false })` wrappers around `PriceChart` and `OracleStatusBadge` with static imports gated by the existing `chartMounted` hook, satisfying the `dynamic-routes-no-ssr-false` Vitest regression guard and preventing the production client-reference manifest corruption (HTTP 500) that broke the stock detail route.
- **2026-05-21 — Stocks portfolio disconnected journey dual-path CTA (task 0048).** Improved `/stocks/portfolio` disconnected flow with explicit helper guidance and a new secondary `Browse Stock Markets` action linking back to `/stocks`, so first-time users can continue research without being forced into an immediate wallet-connect step; added regression coverage in `frontend/src/app/(app)/stocks/portfolio/__tests__/collateral-health.test.tsx`.
