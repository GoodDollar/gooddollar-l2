...
const FALLBACK_STOCKS: Stock[] = [
  ...
  { ticker: 'MSFT', name: 'Microsoft Corp.', sector: 'Technology', description: 'Microsoft Corp. — Windows, Azure, Office.', price: 388.45, change24h: 0.9, volume24h: 22_000_000, marketCap: 2_890_000_000_000, high52w: 420.82, low52w: 309.45, sparkline7d: [383, 384, 385, 386, 387, 388, 388.45], peRatio: 34.2, eps: 11.36, dividendYield: 0.72, avgVolume: 20_000_000 },
  ...
  { ticker: 'AMZN', name: 'Amazon.com', sector: 'Consumer', description: 'Amazon.com — e-commerce & AWS cloud.', price: 186.21, change24h: 1.8, volume24h: 48_000_000, marketCap: 1_950_000_000_000, high52w: 201.20, low52w: 151.61, sparkline7d: [182, 183, 184, 185, 185.5, 186, 186.21], peRatio: 58.3, eps: 3.19, dividendYield: 0, avgVolume: 44_000_000 },
  { ticker: 'NVDA', name: 'NVIDIA Corp.', sector: 'Technology', description: 'NVIDIA Corp. — GPUs & AI computing.', price: 104.75, change24h: 3.2, volume24h: 310_000_000, marketCap: 2_580_000_000_000, high52w: 153.13, low52w: 75.61, sparkline7d: [98, 99, 101, 102, 103, 104, 104.75], peRatio: 60.1, eps: 1.74, dividendYield: 0.03, avgVolume: 280_000_000 },
  { ticker: 'TSLA', name: 'Tesla Inc.', sector: 'Automotive', description: 'Tesla Inc. — electric vehicles & energy.', price: 272.18, change24h: -2.1, volume24h: 95_000_000, marketCap: 874_000_000_000, high52w: 488.54, low52w: 138.80, sparkline7d: [280, 278, 276, 275, 274, 273, 272.18], peRatio: 150.2, eps: 1.81, dividendYield: 0, avgVolume: 88_000_000 },
  ...
  { ticker: 'NFLX', name: 'Netflix', sector: 'Entertainment', description: 'Netflix — streaming entertainment.', price: 998.61, change24h: 1.1, volume24h: 5_200_000, marketCap: 428_000_000_000, high52w: 1040.00, low52w: 550.64, sparkline7d: [990, 992, 994, 995, 996, 997, 998.61], peRatio: 48.9, eps: 20.42, dividendYield: 0, avgVolume: 4_800_000 },
  { ticker: 'AMD', name: 'AMD', sector: 'Technology', description: 'AMD — CPUs, GPUs, adaptive computing.', price: 101.32, change24h: -1.5, volume24h: 42_000_000, marketCap: 164_000_000_000, high52w: 187.28, low52w: 97.09, sparkline7d: [104, 103, 102.5, 102, 101.8, 101.5, 101.32], peRatio: 102.3, eps: 0.99, dividendYield: 0, avgVolume: 38_000_000 },
  { ticker: 'COIN', name: 'Coinbase', sector: 'Finance', description: 'Coinbase Global — crypto exchange platform.', price: 178.54, change24h: 4.2, volume24h: 12_000_000, marketCap: 43_500_000_000, high52w: 349.75, low52w: 146.12, sparkline7d: [170, 172, 174, 175, 176, 177, 178.54], peRatio: 28.6, eps: 6.24, dividendYield: 0, avgVolume: 10_000_000 },
]
...
const STOCK_META: Record<string, { name: string; sector: string; description: string }> = {
  AAPL:  { name: 'Apple Inc.', sector: 'Technology', description: 'Apple Inc. — smartphones, computers, services.' },
  TSLA:  { name: 'Tesla Inc.', sector: 'Automotive', description: 'Tesla Inc. — electric vehicles & energy.' },
  NVDA:  { name: 'NVIDIA Corp.', sector: 'Technology', description: 'NVIDIA Corp. — GPUs & AI computing.' },
  MSFT:  { name: 'Microsoft Corp.', sector: 'Technology', description: 'Microsoft Corp. — Windows, Azure, Office.' },
  AMZN:  { name: 'Amazon.com', sector: 'Consumer', description: 'Amazon.com — e-commerce & AWS cloud.' },
  GOOGL: { name: 'Alphabet Inc.', sector: 'Technology', description: 'Alphabet Inc. — Google, YouTube, Cloud.' },
  META:  { name: 'Meta Platforms', sector: 'Technology', description: 'Meta Platforms — Facebook, Instagram, WhatsApp.' },
  JPM:   { name: 'JPMorgan Chase', sector: 'Finance', description: 'JPMorgan Chase — banking & financial services.' },
  V:     { name: 'Visa Inc.', sector: 'Finance', description: 'Visa Inc. — global payments network.' },
  DIS:   { name: 'Walt Disney', sector: 'Entertainment', description: 'Walt Disney — media, parks, streaming.' },
  NFLX:  { name: 'Netflix', sector: 'Entertainment', description: 'Netflix — streaming entertainment.' },
  AMD:   { name: 'AMD', sector: 'Technology', description: 'AMD — CPUs, GPUs, adaptive computing.' },
  COIN:  { name: 'Coinbase', sector: 'Finance', description: 'Coinbase Global — crypto exchange platform.' },
}
...
export function useOnChainStocks(): { stocks: Stock[]; isLoading: boolean; isLive: boolean } {
  ...
  const stocks = useMemo<Stock[]>(() => {
    ...
    return KNOWN_TICKERS.map((ticker, i) => {
      ...
      const meta = STOCK_META[ticker] ?? { name: `s${ticker}`, sector: 'Unknown', description: `Synthetic ${ticker}` }
