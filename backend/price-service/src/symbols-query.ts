 ...
 * invalid-set partition. Returns `null` when the query is absent OR
 * when every input token trimmed to empty — the bulk `/quotes` handler
 * then runs its legacy unfiltered path verbatim.
 *
 * Pipeline (in order):
