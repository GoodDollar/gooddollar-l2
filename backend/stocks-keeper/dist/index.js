...
exports.TICKERS = exports.OracleUpdater = void 0;
exports.fetchPrices = fetchPrices;
const ethers_1 = require("ethers");
const dotenv_1 = __importDefault(require("dotenv"));
const pino_1 = __importDefault(require("pino"));
dotenv_1.default.config();
const logger = (0, pino_1.default)({ name: 'stocks-keeper' });
// ─── Configuration ───────────────────────────────────────────────────────────
...
async function fetchPrices(tickers) {
    ...
    try {
        ...
        for (const ticker of tickers) {
            try {
                const resp = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${ticker}?range=1d&interval=1m`, {
                    headers: {
 ...
 */
async function fetchFromFallback(ticker) {
    try {
        // Use a simple finance API endpoint
        const resp = await fetch(`https://www.google.com/finance/quote/${ticker}:NASDAQ`, { headers: { 'User-Agent': 'Mozilla/5.0' } });
        if (!resp.ok)
            return null;
...
class OracleUpdater {
    ...
    oracle;
    lastPrices = new Map();
    constructor(rpcUrl, operatorKey, oracleAddress) {
        this.provider = new ethers_1.ethers.JsonRpcProvider(rpcUrl);
        this.wallet = new ethers_1.ethers.Wallet(operatorKey, this.provider);
        this.oracle = new ethers_1.ethers.Contract(oracleAddress, ORACLE_ABI, this.wallet);
    }
    /**
    ...
    async updatePrice(quote) {
        try {
            ...
            const tx = await this.oracle.setManualPrice(quote.ticker, quote.priceChainlink, true, // active = true (use manual price)
            { gasLimit: 100_000 });
...
exports.OracleUpdater = OracleUpdater;
// ─── Main Loop ───────────────────────────────────────────────────────────────
function sleep(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
}
async function runCycle(updater) {
