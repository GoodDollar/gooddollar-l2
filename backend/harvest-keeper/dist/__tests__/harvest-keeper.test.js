...
describe('HarvestKeeper', () => {
    let keeper;
    beforeEach(() => {
        keeper = new lib_1.HarvestKeeper(testConfig);
    });
    ...
    describe('config defaults', () => {
        test('default interval is 3600s', () => {
            expect(lib_1.DEFAULT_CONFIG.minHarvestIntervalSeconds).toBe(3600);
        });
        test('default min profit is 10 BPS', () => {
            expect(lib_1.DEFAULT_CONFIG.minProfitThresholdBPS).toBe(10);
        });
        test('default dry run is false', () => {
            expect(lib_1.DEFAULT_CONFIG.dryRun).toBe(false);
        });
    });
...
describe('HarvestKeeper integration (dry run)', () => {
    test('constructs with valid config', () => {
        const keeper = new lib_1.HarvestKeeper(testConfig);
    ...
    test('config merges with defaults', () => {
        ...
        const keeper = new lib_1.HarvestKeeper(partialConfig);
        const vault = mockVault({
