...
class BridgeMonitor {
    ...
    constructor(config) {
        ...
        if (config.lifiAggregatorAddress) {
            this.lifiAggregator = new ethers_1.ethers.Contract(config.lifiAggregatorAddress, config.lifiAggregatorAbi, config.wallet);
        }
        if (config.multiChainBridgeAddress) {
            this.multiChainBridge = new ethers_1.ethers.Contract(config.multiChainBridgeAddress, config.multiChainBridgeAbi, config.wallet);
        }
    }
    // ─── Start / Stop ─────────────────────────────────────────────────────────
    startSwapMonitor() {
    ...
    async processNextPendingSwap() {
        ...
        for (const swap of this.pendingSwaps.values()) {
            if (swap.status === 'pending') {
                if (!oldest || swap.swapId < oldest.swapId)
                    oldest = swap;
            }
        }
        if (!oldest)
        ...
        try {
            ...
            const txHash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(`lifi-${swap.swapId}-${Date.now()}`));
            await this.completeSwap(swap.swapId, txHash, destAmount);
    ...
    async completeSwap(swapId, lifiTxHash, destAmount) {
        if (!this.lifiAggregator)
            throw new Error('No aggregator contract');
    ...
    async refundSwap(swapId, reason) {
        if (!this.lifiAggregator)
            throw new Error('No aggregator contract');
...
exports.BridgeMonitor = BridgeMonitor;
