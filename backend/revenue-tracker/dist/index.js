...
exports.PROTOCOLS = exports.RevenueReporter = void 0;
...
const logger = (0, pino_1.default)({ name: 'revenue-tracker' });
// ─── Configuration ───────────────────────────────────────────────────────────
 ...
 */
async function collectProtocolFees(provider, protocol) {
    let totalFees = 0n;
    let txCount = 0n;
    for (const contractDef of protocol.contracts) {
        try {
            const contract = new ethers_1.ethers.Contract(contractDef.address, contractDef.abi, provider);
...
class RevenueReporter {
    ...
    tracker;
    lastSnapshotTime = 0;
    constructor(rpcUrl, operatorKey, trackerAddress) {
        this.provider = new ethers_1.ethers.JsonRpcProvider(rpcUrl);
        this.wallet = new ethers_1.ethers.Wallet(operatorKey, this.provider);
        this.tracker = new ethers_1.ethers.Contract(trackerAddress, TRACKER_ABI, this.wallet);
    }
    /**
    ...
    async init() {
        try {
            ...
            logger.info({
                ...
                totalFees: ethers_1.ethers.formatEther(totalFees),
                totalUBI: ethers_1.ethers.formatEther(totalUBI),
            ...
            for (let i = 0; i < Number(count); i++) {
                ...
                logger.info({
                    ...
                    name: p.name,
                    category: p.category,
                    totalFees: ethers_1.ethers.formatEther(p.totalFees),
                    ubiContribution: ethers_1.ethers.formatEther(p.ubiContribution),
    ...
    async runCycle() {
        ...
        for (const protocol of PROTOCOLS) {
            try {
                ...
                const tx = await this.tracker.reportFees(protocol.id, delta.fees, delta.ubi, delta.txs, { gasLimit: 200_000 });
                ...
                logger.info({
                    ...
                    fees: ethers_1.ethers.formatEther(delta.fees),
                    ubi: ethers_1.ethers.formatEther(delta.ubi),
    ...
    async printStatus() {
        ...
        console.log(`Total Fees Tracked: ${ethers_1.ethers.formatEther(totalFees)} G$`);
        console.log(`Total UBI Funded:   ${ethers_1.ethers.formatEther(totalUBI)} G$`);
        console.log(`Total Transactions: ${totalTx}`);
        console.log('');
        for (let i = 0; i < Number(count); i++) {
            const p = await this.tracker.getProtocol(i);
            console.log(`  [${i}] ${p.name} (${p.category}): ${ethers_1.ethers.formatEther(p.totalFees)} fees, ${ethers_1.ethers.formatEther(p.ubiContribution)} UBI, ${Number(p.txCount)} txs`);
        }
        console.log('═══════════════════════════════════\n');
    }
}
exports.RevenueReporter = RevenueReporter;
// ─── Main Loop ───────────────────────────────────────────────────────────────
function sleep(ms) {
